...
class Protocolo
{
    ...
    public function IsMyProtocoloForwarding($id, $meuSetor)
    {
        ...
        $result = mysqli_fetch_assoc($db->Query("SELECT COUNT(*) as total FROM `encaminhamento` WHERE `protocolo_id` = $id AND `remetente_id` = $meuSetor  ORDER BY `data` DESC LIMIT 1;"));
